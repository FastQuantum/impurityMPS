...
Tests use Catch2 v2, in six executables. `fbr_test` holds the fast unit tests (`test_givens`, `test_graph`, `test_fb_mps`, `test_green_overlap`, ...). The other five need their own executable, because the TDVP headers define non-inline functions. Four compare a solver against a committed reference: `fbr_test_fbr` (spin-symmetric), `fbr_test_block` (generic spin), `fbr_test_ns` (spinless) and `fbr_test_green` (Green functions of the SIAM and the IRLM against their chain baselines, L=100). The fifth, `fbr_test_green_sep`, checks the separate-frame Green function against the exact U=0 result. The reference runs stop early by default (t=5 for the correlators, t=2 for the Green functions); configure with `-DFBR_ENABLE_LONG_TEST=ON` to go to t=20. The parameters covered are L=100 and V=0.1, with U=0.1 and 0.2 for the SIAM (correlators and Green function), and U=0.1, 0.2 and -0.2 for the IRLM Green functions.
